...
class UserService {
    ...
    async getUsers(params) {
        ...
        // 搜索过滤
        if (keyword) {
            users = users.filter(user => user.username.toLowerCase().includes(keyword.toLowerCase()) ||
        ...
        // 角色过滤
        if (role) {
            users = users.filter(user => user.roles.some(r => r.code === role));
        }
        // 排序
        users.sort((a, b) => {
    ...
    async createUser(userData) {
        ...
        if (!userData.username || !userData.email || !userData.password) {
            throw new error_1.ValidationError('用户名、邮箱和密码不能为空');
        }
        if (!this.validateEmail(userData.email)) {
            throw new error_1.ValidationError('邮箱格式不正确');
        }
        if (userData.password.length < 6) {
            throw new error_1.ValidationError('密码长度至少6位');
        }
        // 检查用户名和邮箱唯一性
        const existingUser = await this.userRepository.findByUsernameOrEmail(userData.username, userData.email);
        if (existingUser) {
            throw new error_1.ValidationError('用户名或邮箱已存在');
        }
        // 密码加密
    ...
    async updateUser(id, userData) {
        ...
        if (userData.email) {
            ...
            if (existingUser && existingUser.id !== id) {
                throw new error_1.ValidationError('邮箱已存在');
            }
        }
        // 更新用户
    ...
    async deleteUser(id) {
        ...
        if (user && user.roles.some(role => role.code === 'SUPER_ADMIN')) {
            throw new error_1.ValidationError('不能删除超级管理员');
        }
        return await this.userRepository.delete(id);
    }
    async importUsers(buffer) {
        // 这里应该实现Excel解析逻辑
        // 简化实现
        return {
            success: 0,
