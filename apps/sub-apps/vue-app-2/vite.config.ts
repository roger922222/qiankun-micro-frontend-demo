...
export default defineConfig({
  ...
  base: process.env.NODE_ENV === 'production' ? '/vue-file-management/' : '/',
  
  css: {
    preprocessorOptions: {
      // 确保样式在微前端环境下正确处理
      scss: {
        additionalData: `$qiankun-prefix: '.vue-file-management';`
      }
    },
    // 开发环境样式处理优化
    devSourcemap: true,
      ...
      plugins: [
        {
          ...
          Once(root) {
            // 为所有样式规则添加更高的优先级
            root.walkRules(rule => {
              if (rule.selector.includes('.vue-file-management')) {
                rule.selector = rule.selector.replace(/\.vue-file-management/g, '.vue-file-management');
              }
            });
          }
        }
