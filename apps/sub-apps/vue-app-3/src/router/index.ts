...
const routePerformance = {
  ...
  start() {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      const existingData = JSON.parse(sessionStorage.getItem('vue-app-3-route-performance') || '[]');
      existingData.push(perfData);
      sessionStorage.setItem('vue-app-3-route-performance', JSON.stringify(existingData));
    }
  }
};
...
const routes: RouteRecordRaw[] = [
  ...
  },
  {
    ...
    component: createLazyComponent(
      () => import('../views/SystemDashboard.vue'),
      'SystemDashboard'
    ),
    meta: {
      title: '系统概览',
  ...
  },
  {
    ...
    name: 'ServiceMonitor',
    component: createLazyComponent(
      () => import('../views/ServiceMonitor.vue'),
      'ServiceMonitor'
    ),
    meta: {
      title: '服务监控',
...
export const getRoutePerformanceData = () => {
  if (typeof window !== 'undefined') {
    return JSON.parse(sessionStorage.getItem('vue-app-3-route-performance') || '[]');
  }
  return [];
};
...
export const clearRoutePerformanceData = () => {
  if (typeof window !== 'undefined') {
    sessionStorage.removeItem('vue-app-3-route-performance');
  }
};
