...
export default defineConfig({
  ...
  base: process.env.NODE_ENV === 'production' ? '/vue-system-monitor/' : '/',
  
  css: {
    preprocessorOptions: {
      // 确保样式在微前端环境下正确处理
      scss: {
        additionalData: `$qiankun-prefix: '.vue-system-monitor';`
      }
    },
    // 开发环境样式处理优化
    devSourcemap: true,
      ...
      plugins: [
        {
          ...
          Once(root) {
            // 为所有样式规则添加更高的优先级
            root.walkRules(rule => {
              if (rule.selector.includes('.vue-system-monitor')) {
                rule.selector = rule.selector.replace(/\.vue-system-monitor/g, '.vue-system-monitor');
              }
            });
          }
        }
