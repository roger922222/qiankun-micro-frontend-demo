<template>
  <div class="vue-message-center" v-if="isComponentReady">
    <div class="message-center">
      ...
      <a-row :gutter="[16, 16]" class="message-content-row">
        <a-col :xs="24" :sm="24" :md="10" :lg="8" :xl="8">
          <a-card title="消息列表" size="small" class="message-list-card">
            <template #extra>
        ...
        <a-col :xs="24" :sm="24" :md="14" :lg="16" :xl="16">
          <a-card title="消息详情" size="small" class="message-detail-card">
...
onMounted(async () => {
  try {
    ...
    // 通知父组件组件已就绪
    emit('component-ready', 'MessageCenter');
  } catch (error) {
    console.error('MessageCenter 初始化失败:', error);
    // 降级处理：延迟后仍然显示组件
    setTimeout(() => {
      isComponentReady.value = true;
      emit('component-ready', 'MessageCenter');
